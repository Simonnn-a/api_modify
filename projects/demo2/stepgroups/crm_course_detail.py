    # -*- coding=utf-8 -*-
"""
crm_course_detailsuccess运行期望成功的case
crm_course_detailfaild运行期望失败的case
crm_course_detail_api_relevance运行接口关联的case（需要手动修改需要存入/取出的字段值）
...
class Coursedetail():
    ...
    def crm_course_detailSuccess(self,parameterID):
    ...
    def crm_course_detailFaild(self,parameterID):
        stepParameters = ParameterHelper.getStepParameters(parameterID)
        parameters = stepParameters['Parameters']
        ...
        print("返回值 = " + str(res_dict))
        AssertHelper.assert_dict_contain_after_logging(expect_result, res_dict)
    def crm_course_detail_Api_relevance(self,parameterID):
        stepParameters = ParameterHelper.getStepParameters(parameterID)
        parameters = stepParameters['Parameters']
